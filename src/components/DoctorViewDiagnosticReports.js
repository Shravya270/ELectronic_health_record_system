...
const DoctorViewDiagnosticReports = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Get patient details and address
        ...
        try {
          const permission = await uploadEhrContract.methods
            .patientToDoctorPermission(patientAddress, currentAccount)
            .call();

          setHasPermission(permission);

          if (permission) {
            // Fetch diagnostic reports for doctor (with permission check)
            const diagnosticReports = await uploadEhrContract.methods
              .getDiagnosticReportsForDoctor(patientAddress)
            ...
            setError("Access Denied: Patient has not granted permission to view diagnostic reports.");
          }
        } catch (permissionError) {
          console.error("Error checking permission:", permissionError);
          if (permissionError.message.includes("Permission not granted")) {
            setHasPermission(false);
            setError("Access Denied: Patient has not granted permission to view diagnostic reports.");
          } else {
            setError("Error checking permission: " + permissionError.message);
          }
        }
      } catch (error) {
      ...
      <div>
        ...
        <div className="bg-gradient-to-b from-black to-gray-800 text-white p-10 font-mono min-h-screen">
          <div className="max-w-2xl mx-auto">
            <h1 className="text-3xl font-bold mb-6 text-teal-400">Diagnostic Reports</h1>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {patientDetails && (
            <div className="bg-gray-900 p-4 rounded-lg mb-6 border border-gray-700">
              <p className="text-gray-300">
                <span className="font-bold text-teal-400">Patient:</span> {patientDetails.name}
              </p>
            </div>
          )}
          ...
          {reports.length === 0 ? (
            <div className="bg-gray-900 p-8 rounded-lg border border-gray-700 text-center">
              <p className="text-gray-400 text-lg">No diagnostic reports found for this patient.</p>
                ...
                >
                  ...
                  <div className="mt-4">
                    <a
                      ...
                      className="block w-full px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-lg text-center transition"
                    >
                      View Report
                    </a>
                  </div>
...
export default DoctorViewDiagnosticReports;
